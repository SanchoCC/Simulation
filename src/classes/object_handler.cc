...
void ObjectHandler::Rotate(Object* object, float delta_time) {
    if (object->GetStatical()) {
        ...
        if (rotation_angle > 2.0f * M_PI || rotation_angle < -2.0f * M_PI) {
            object->SetRotationAngle(rotation_angle - 2.0f * M_PI * static_cast<float>(static_cast<int>(rotation_angle / (2.0f * M_PI))));
        }
    }
}
...
CollisionResult ObjectHandler::CheckCollision(const Object* object1, const Object* object2) const {
    ...
    } else if (object1->GetType() == ShapeType::kCircle && object2->GetType() == ShapeType::kRectangle) {
        return CircleRectangle(object1, object2);
    } else if (object1->GetType() == ShapeType::kRectangle && object2->GetType() == ShapeType::kCircle) {
...
CollisionResult ObjectHandler::SATCollision(const Object* object1, const Object* object2) const {
    ...
    auto projectVertices = [](const std::vector<glm::vec2>& vertices, const glm::vec2& axis) {
...
CollisionResult ObjectHandler::CircleCircle(const Object* object1, const Object* object2) const {
    ...
    if (distance == 0.0f) {
        ...
        result.contacts.push_back(result.normal * circle1->GetRadius() + object1->GetPosition());
    }
    return result;
}
...
void ObjectHandler::HandleCollision(Object* object1, Object* object2, CollisionResult collision_result) {
    ...
    for (int i = 0; i < collision_result.contacts.size(); ++i) {
        ...
        glm::vec2 relative_velocity = velocity2 + sim::Cross2D(angular_velocity2, r2) - velocity1 - sim::Cross2D(angular_velocity1, r1);
        ...
        float inverse_mass_sum = object1->GetInvertedMass() + object2->GetInvertedMass() + (r1_cross_normal * r1_cross_normal) * object1->GetInvertedInertia() + (r2_cross_normal * r2_cross_normal) * object2->GetInvertedInertia();

        float restitution_sum = std::min(object1->GetRestitution(), object2->GetRestitution());
        ...
        relative_velocity = velocity2 + sim::Cross2D(angular_velocity2, r2) - velocity1 - sim::Cross2D(angular_velocity1, r1);

        glm::vec2 tangent = relative_velocity - (normal * glm::dot(relative_velocity, normal));
...
//glm::vec2 ObjectHandler::FindSupportVertex(Object* object, glm::vec2 direction) {
